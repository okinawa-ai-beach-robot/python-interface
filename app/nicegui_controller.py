    ...
    try:
        if tab_panel.value==tab_names[1]:
            if live_update_timer is None:
                live_update_timer=ui.timer(interval=0.5, callback=lambda: video_image.set_source(f'/video/frame?{time.time()}'))
        else:
...
with tab_panel:
    with ui.tab_panel(one):
        with ui.row():
            toggle1 = ui.toggle({1: 'Video Stop', 2: 'Record'}, value=1).on_value_change(lambda v: toggle_recoding(v.value==2))
...
''')
        ui.joystick(color='blue', size=350,
            on_move=lambda e: joystick_move(e),
            on_end=lambda _: joystick_end()).classes('custom-joystick')
