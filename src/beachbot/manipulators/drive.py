...
class DifferentialDrive(threading.Thread):
    ...
    def run(self):
        ...
        while self._is_running:
            ...
            dir_dot = dir_dir * min(self._max_rate_of_change/self.update_freq, abs(dir_delta))
            vel_dot = vel_dir * min(self._max_rate_of_change/self.update_freq, abs(vel_delta))

            self._current_angular_vel =  bounded(self._current_angular_vel+dir_dot, -100, 100)
            self._current_velocity =  bounded(self._current_velocity+vel_dot, -100, 100)
            ...
            __motor_right_speed = bounded(sign(__motor_right_speed)*15 + __motor_right_speed, -100, 100)
